...
class Uniswap(Client):
    ...
    def __init__(self, private_key, web3, chain_id, log, target_price_impact):
        ...
        self.router = UNISWAP_CONTRACT[self.chain_id]["router"]
        self.quoter = UNISWAP_CONTRACT[self.chain_id]["quoter"]
        self.router_abi = js.load(open("abi/uniswap_router.json"))
        self.quoter_abi = js.load(open("abi/uniswap_quoter.json"))
    ...
    def buy_token(self, token_name, token_hash, balance_percent, retry=1):
        self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Обмениваю ETH на {token_name}')
        try:
            balance = self.web3.eth.get_balance(self.wallet_address)
            amount = int(balance * (balance_percent / 100))
            if amount <= 0:
                self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Недостаточно средств')
                retry += 1
                if retry > 3:
                    return False
            ...
            self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Проверяю price impact')
            ...
            self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Отправил транзакцию')
            tx_receipt = self.web3.eth.wait_for_transaction_receipt(tx_hash, timeout=30)
            if tx_receipt.status == 1:
                hash_ = str(tx_hash.hex())
                self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Куплено минимум {bought_amount} '
                              f'{token_name} | Хэш транзакции: {hash_}')
            else:
                self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Транзакция не прошла')
                retry += 1
                if retry > 3:
                    return False
                return self.buy_token(token_name, token_hash, balance_percent, retry)
            ...
            self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Транзакция не прошла за долгий '
            ...
            self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Ошибка подключения к интернету или '
                          f'проблемы с RPC')
            retry += 1
            if retry > 3:
                return False
            ...
            self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Ошибка ({error})')
            retry += 1
            if retry > 3:
                return False
            return self.buy_token(token_name, token_hash, balance_percent, retry)
    ...
    def sell_token(self, token_name, token_hash, retry=1):
        self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Обмениваю {token_name} на ETH')
        try:
            ...
            if token_balance == 0:
                self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Баланс {token_name} - 0')
                retry += 1
                if retry > 3:
                    return False
            ...
            if allowance < 100000 * 10 ** decimal:
                self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Делаю approve')
            ...
            if approved:
                ...
                if tx_receipt.status == 1:
                    hash_ = str(tx_hash.hex())
                    self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Продано {bought_amount} '
                                  f'{token_name} | Хэш транзакции: {hash_}')
                else:
                    self.log.info(f'{self.chain_name} | {self.wallet_address} | Uniswap | Транзакция не прошла')
                    retry += 1
